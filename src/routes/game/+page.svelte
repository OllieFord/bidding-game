	...
	function calculateChargeState(index, state) {
		...
		if (state == '3') {
			if (currentChargeState <= 0 || currentChargeState > 50) {
				...
				estimatedRevenue.upper[priceIndex] =
					-data[priceIndex].fcr_n_price_upper * fineMultiplication;
				estimatedRevenue.lower[priceIndex] =
					-data[priceIndex].fcr_n_price_lower * fineMultiplication;
				estimatedRevenue.value[priceIndex] =
					(estimatedRevenue.upper[priceIndex] + estimatedRevenue.lower[priceIndex]) / 2;
				return currentChargeState;
			} else {
				// No negation applied
		...
		<div class="relative w-full max-w-7xl max-h-full">
			...
			<div class="relative bg-white rounded-lg shadow">
				...
				<div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
					<h3 class="text-xl font-medium text-gray-900">RULES</h3>
					<button
						type="button"
						class="text-gray-400 p-4 bg-transparent hover:scale-105 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
				...
				<div class="p-4 md:p-5 space-y-4">
					...
					<p class="text-base leading-relaxed text-gray-500">
						You want to generate as much revenue as possible for the whole day with a battery energy
						storage system (BESS).
					</p>
					<p class="text-base leading-relaxed text-gray-500">
						You have a 1MW/2MWh BESS which is at 0% state of charge (SoC) in the beginning of the
						day. Each hour (PTU) you have 4 options: (1) sell it to FCR-N, (2) charge or (3)
						discharge it with the day-ahead electricity prices, or (4) do nothing. There are 24
						hourly PTUs.
					</p>
					...
					<p class="text-base leading-relaxed text-gray-500">
						...
						PTU. With this BESS, you can sell 1MW on FCR-N. If you sell it to FCR-N the SoC must be at
						50%, otherwise you will pay a penalty of 3x the price. After the FCR-N hour the battery will
						be at 50% SoC. There are no energy costs for selling it to FCR-N.
					</p>
					<h3 class="text-md font-medium">CHARGE</h3>
					<p class="text-base leading-relaxed text-gray-500">
						You receive the <i>known day-ahead electricity (SPOT) price</i> for each PTU. With this BESS,
						you can charge and pay this price for 1MWh each PTU. If you charge the BESS during a PTU,
						the SoC will increase by 50%, so you can charge the BESS for 2 consecutive PTUs if the BESS
						is at 0% in the beginning of the first hour. If you charge the BESS and the SoC is already
						100%, nothing happens during that PTU, and you pay/receive nothing.
					</p>
					<h3 class="text-md font-medium">DISCHARGE</h3>
					<p class="text-base leading-relaxed text-gray-500">
						You receive the <i>known day-ahead electricity (SPOT) price</i> for each PTU. With this BESS,
						you can discharge and receive this price for 1MWh each PTU. If you discharge the BESS during
						a PTU, the SoC will decrease by 50%, so you can discharge the BESS for 2 consecutive PTUs
						if the BESS is at 100% in the beginning of the first hour. If you discharge the BESS and
						the SoC is already at 0%, nothing happens during that PTU, and you pay/receive nothing.
					</p>
					<h3 class="text-md font-medium">NOTHING</h3>
					<p class="text-base leading-relaxed text-gray-500">You can also choose to not to bid.</p>
				</div>
			</div>
		</div>
	</div>
...
<Modal bind:showModal>
	<h2 slot="header" class="font-medium text-xl">Submit Bids</h2>

	<div class="flex flex-row px-8 pt-8 pb-4 justify-center items-center">
		<div style="width: auto; height: 100%;">
			<svg viewBox="0 0 20 32" fill="none" xmlns="http://www.w3.org/2000/svg" height="64"
		...
		<div style="width: auto; height: 100%;">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 32" fill="none" height="64"
	...
	<div class="flex flex-row justify-center items-center">Your Score</div>
	<form class="py-4 px-2 space-y-4" method="POST" use:enhance action="?/addScore">
		{#if form?.missing}<p class="error red">Please fill in the missing values.</p>{/if}
		...
		<div class="flex flex-row justify-end">
			<button
				class="text-white bg-black hover:scale-105 focus:ring-4 transition-all focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
				formaction="?/addScore">Submit</button
			>
		</div>
	</form>
</Modal>
...
<main class="hidden sm:block w-full">
	<div class="flex flex-col sm:flex-row w-full flex-grow h-vh">
		<div class="flex flex-col pr-4 min-h-[488px] h-auto space-y-4">
			<div class="px-4 bg-white rounded-md py-4 light-border">
			...
			<!-- <div class="  bg-white rounded-md light-border">
				<div class="w-full h-full relative rounded-md overflow-hidden">
					<Timer />
				</div>
			</div> -->
			...
			<div class="flex flex-row gap-4">
				<button
					class="px-4 py-3 z-50 w-full bg-white border border-gray-300 hover:scale-105 hover:text-neurtal-800 transition-all rounded-lg shadow-lg text-neurtal-800 font-medium tracking-wide"
					onclick="modal.showModal()">RULES</button
				>
				<button
					class="px-4 py-3 z-50 w-full bg-red hover:scale-105 rounded-lg shadow-lg transition-all text-white font-bold tracking-wide"
			...
			<div class="rounded-lg">
				<button
					class="px-4 py-3 bg-blue-700 hover:scale-105 rounded-lg shadow-lg text-white font-normal transition-all tracking-wide"
			...
			<div class="hidden sm:block !mt-auto text-xs text-[#50565e] z-50 pb-4 -mr-2">
				<div class="flex-grow flex flex-col">
					<div class="flex flex-grow items-center justify-end h-6">Revenue</div>

					<div class="flex flex-grow items-center justify-end h-[19.3px] mb-[3.5px]">No Bid</div>
					<div class="flex flex-grow items-center justify-end h-[19.3px] mb-[3.5px]">Charge</div>
					<div class="flex flex-grow items-center justify-end h-[19.3px] mb-[3.5px]">Discharge</div>
					<div class="flex flex-grow items-center justify-end h-[19.3px]">FCR-N</div>
				</div>
			</div>
		</div>
		...
		<div class="flex flex-col flex-grow">
			<div
				class=" overflow-x-scroll sm:overflow-auto flex flex-col flex-grow h-vh bg-white p-4 rounded-md light-border pb-2"
			>
			...
			<div
				class=" overflow-x-scroll sm:overflow-auto flex flex-col flex-grow h-vh bg-white p-4 rounded-md mt-4 pb-2 light-border"
			>
				<div class="h-44">
			...
			<div class="flex flex-col flex-grow bg-white p-4 rounded-md light-border mt-4">
				<div class="mb-2">
					<div class="font-bold uppercase tracking-tight">Bidding Controls</div>
					<div class="font-medium text-sm text-gray-600 tracking-tight">
						Click on the squares to make a bid
					</div>
				</div>
				<div class="flex flex-row flex-grow justify-between pl-[1.9rem] pr-[2.8rem] col-span-4">
					{#each ptus as ptu, index}
						<div class=" border-gray-300 text-center flex flex-col text-[10px] gap-1">
							<div class="mb-4 font-medium tracking-tight font-mono">
								{#if ptu < 10}
									0{ptu}
								{:else}
									{ptu}
							...
							<fieldset class="relative">
								{#if estimatedRevenue.value[index].toFixed(0) < 0}
									<div
										class="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-full font-bold pb-1 red text-xs leading-tight whitespace-nowrap z-10"
									>
										{estimatedRevenue.value[index].toFixed(0)}
									</div>
								{:else if estimatedRevenue.value[index].toFixed(0) > 0}
									<div
										class="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-full font-bold pb-1 green text-xs leading-tight whitespace-nowrap z-10"
									>
										{estimatedRevenue.value[index].toFixed(0)}
									</div>
								{:else}
									<div
										class="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-full font-bold pb-1 text-xs leading-tight whitespace-nowrap z-10"
									>
										{estimatedRevenue.value[index].toFixed(0)}
									</div>
								{/if}
								...
								<div>
									<input
										...
										name={`select-${ptu}`}
										value="0"
										bind:group={selectedValues[ptu + 1]}
										on:change={() => updateSelectedValue(ptu, '0')}
										checked
									/>
									<label
										class="radio-label inline-block p-1 !rounded-sm action-select-button cursor-pointer"
										for={`no-bid-${ptu}`}
									>
										<div class="h-2 w-2 rounded-none"></div>
									</label>
								</div>
								<div>
									<input
										...
										value="1"
										bind:group={selectedValues[ptu + 1]}
										on:change={() => updateSelectedValue(ptu, '1')}
									/>
									<label
										class="radio-label inline-block p-1 !rounded-sm action-select-button cursor-pointer"
										for={`charge-${ptu}`}
								...
								<div>
									<input
										...
										value="2"
										bind:group={selectedValues[ptu + 1]}
										on:change={() => updateSelectedValue(ptu, '2')}
									/>
									<label
										class="radio-label inline-block p-1 !rounded-sm action-select-button cursor-pointer"
										for={`discharge-${ptu}`}
								...
								<div>
									<input
										...
										value="3"
										bind:group={selectedValues[ptu + 1]}
										on:change={() => updateSelectedValue(ptu, '3')}
									/>
									<label
										class="radio-label inline-block p-1 !rounded-sm action-select-button cursor-pointer"
										for={`fcrn-${ptu}`}
